#!/usr/bin/env python3
"""Fail the build if a distribution leaks private/ design-docs or the local-only wtf tree.

The privacy guarantee for dazzlecmd's PyPI artifacts is the clean-checkout build:
``private/`` design-doc trees and the ``wtf`` recursion-proof aggregator are
gitignored, so a fresh CI checkout simply does not contain them. This script is the
enforcing backstop -- it scans the actually-built sdist (``.tar.gz``) and wheel
(``.whl``) for any path that must never ship and exits non-zero so the publish
workflow aborts before ``twine upload``.

It is depth-proof by construction: it matches path *components* (``private`` or
``wtf`` anywhere in a member path), not MANIFEST.in globs, so nested content cannot
...
# A member path is forbidden if any of its components is one of these directory
# names, or its basename is the local-only wtf kit pointer.
FORBIDDEN_DIRS = {"private", "wtf"}
FORBIDDEN_BASENAMES = {"wtf.kit.json"}
...
def main(argv):
    ...
    for art in artifacts:
        ...
        if bad:
            ...
            print("OK:   %s (no private/ or wtf paths)" % art)
    if total_leaks:
