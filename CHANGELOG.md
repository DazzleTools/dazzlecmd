...
### Fixed
- PyPI publishing is now idempotent (`skip-existing: true` on both publish jobs): the `dazzle-dz` alias moves slower than the main package, so a tag push re-offering an already-published alias file is expected -- it failed the workflow's second-ever run (`400 File already exists` on `dazzle_dz-0.12.0a1`) instead of skipping. The alias also locksteps to `0.12.4a1` with its dazzlecmd floor raised to `0.12.4a0`.
...
- **`dz kit -h` now ends with a contextual `TIP:` footer.** Tips are scoped by detail level — the standard tip (e.g. "every axis takes the universal poles — `on`==warm, `off`==cold") shows by default, while more advanced tips are reserved for a higher detail level. This is the first use of the shared tip system across the CLI-presentation libraries; richer detail-scoped tips (driven by `-v`/`--detail`) follow.
