...
# A known timestamp to stamp the synthetic source with (distinct from "now"),
# so we can verify it survives the symlink operations. Self-contained -- the
# test creates its own source file, with no dependency on any real on-disk file.
SOURCE_TIMESTAMP = datetime.datetime(2023, 1, 1, 9, 0, 0).timestamp()
...
def main():
    ...
    # --- Step 1: Set up a self-contained source file ---
    if TEST_DIR.exists():
        shutil.rmtree(TEST_DIR)
    TEST_DIR.mkdir(parents=True)

    # Create a synthetic source file with a known (non-"now") timestamp, so the
    # test depends on nothing but itself.
    target_copy = TEST_DIR / "original_file.md"
    target_copy.write_text("# symlink-timestamp-test fixture\n", encoding="utf-8")
    os.utime(target_copy, (SOURCE_TIMESTAMP, SOURCE_TIMESTAMP))
    print(f"\nStep 1: Created synthetic source at {target_copy}")
    ...
    # --- Restore target timestamps to the known source timestamp ---
    os.utime(str(target_copy), (SOURCE_TIMESTAMP, SOURCE_TIMESTAMP))
    print("\n  (Restored target file timestamps to the source timestamp)")
